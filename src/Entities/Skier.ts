...
/**
 * Skier can jump this obstacles
 */
const LOW_OBSTACLES = [IMAGE_NAMES.ROCK1, IMAGE_NAMES.ROCK2, IMAGE_NAMES.JUMP_RAMP];

/**
 * This is a list of obstacles that triggers jump
 */
const JUMPERS = [IMAGE_NAMES.JUMP_RAMP];
 ...
 export class Skier extends Entity {
     ...
     */
     setupAnimations() { 
         this.animations[STATES.STATE_JUMPING] = new Animation(JUMPING_IMAGES, false, () => {
    ...
    isJumping(): boolean {
     ...
     update(gameTime: number) {
         if (this.isJumping()) {
            this.curAnimation = this.animations[STATES.STATE_JUMPING];
     ...
     checkIfHitObstacle() {
         ...
         if (collision) {
            if (JUMPERS.indexOf(collision.imageName) >= 0) {
                this.jump();
            }
             
            if (LOW_OBSTACLES.indexOf(collision.imageName) >= 0 && this.isJumping()) {
                return;
            }
